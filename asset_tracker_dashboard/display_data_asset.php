...
function get_dashboard_metrics(): array
{
    ...
    if (!$tentResult || !$vehicleResult) {
        throw new RuntimeException('Dashboard metrics query failed.');
    }

    $tent = mysqli_fetch_assoc($tentResult);
    $vehicle = mysqli_fetch_assoc($vehicleResult);
    $tentTotal = (int) ($tent['total'] ?? 0);
    $vehicleTotal = (int) ($vehicle['total'] ?? 0);

    return [
        'tent_total' => $tentTotal,
        'tent_available' => (int) ($tent['available'] ?? 0),
        'tent_installed' => (int) ($tent['installed'] ?? 0),
        'tent_for_retrieval' => (int) ($tent['for_retrieval'] ?? 0),
        'tent_long_term' => (int) ($tent['long_term'] ?? 0),
        'tent_available_percent' => $tentTotal > 0 ? (int) round(((int) $tent['available'] / $tentTotal) * 100) : 0,
        'vehicle_total' => $vehicleTotal,
        'vehicle_available' => (int) ($vehicle['available'] ?? 0),
        'vehicle_departed' => (int) ($vehicle['departed'] ?? 0),
        'vehicle_available_percent' => $vehicleTotal > 0 ? (int) round(((int) $vehicle['available'] / $vehicleTotal) * 100) : 0,
        'vehicle_departed_percent' => $vehicleTotal > 0 ? (int) round(((int) $vehicle['departed'] / $vehicleTotal) * 100) : 0,
    ];
}
...
function  display_tent_status_wake()
{
    global $conn;

    // Query to get count of 'On Stock' tents
    $query = "SELECT COUNT(*) AS stock_count FROM `tent` WHERE `Status` = 'Retrieved'  AND `purpose` = 'Wake'";
    $result = mysqli_query($conn, $query);
    if (!$result) {
        die("Query failed: " . mysqli_error($conn));
    }
