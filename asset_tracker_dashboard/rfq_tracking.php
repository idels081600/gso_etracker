...
if (!$result1) {
    error_log('RFQ query failed: ' . mysqli_error($conn));
    die('Unable to load RFQ records.');
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST"  && isset($_POST['save_data'])) {
    ...
    asset_validate_csrf();
    try {
        $rfqNo = input_string($_POST, 'rfqno', 80);
        $prNo = input_string($_POST, 'prno', 80);
        $rfqName = input_string($_POST, 'rfqname', 255);
        ...
        $date = date('Y-m-d');
        $stmt = db_execute($conn, "INSERT INTO RFQ (rfq_no, pr_no, rfq_name, date, amount, requestor, supplier, Status) VALUES (?, ?, ?, ?, ?, ?, ?, 'Office Clerk')", 'ssssdss', [$rfqNo, $prNo, $rfqName, $date, $amount, $requestor, $supplier]);
        mysqli_stmt_close($stmt);
        header('Location: rfq_tracking.php?success=1');
        exit;
    } catch (Throwable $error) {
        error_log($error->getMessage());
        header('Location: rfq_tracking.php?error=insert_failed');
        exit;
    }
}
                            ...
                            <tbody>
                                <?php while ($row = mysqli_fetch_assoc($result1)) { ?>
                                    <tr class="clickable-row" data-rfq-id="<?php echo $row["id"]; ?>">
                                        <td data-column="rfq_no"><?php echo e($row["rfq_no"]); ?></td>
                                        <td data-column="pr_no"><?php echo e($row["pr_no"]); ?></td>
                                        <td data-column="rfq_name"><?php echo e($row["rfq_name"]); ?></td>
                                        <td data-column="date"><?php echo e($row["date"]); ?></td>
                                        <td data-column="amount"><?php echo e($row["amount"]); ?></td>
                                        <td data-column="requestor"><?php echo e($row["requestor"]); ?></td>
                                        <td data-column="supplier"><?php echo e($row["supplier"]); ?></td>
