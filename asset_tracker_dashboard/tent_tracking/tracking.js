$(function () {
  ...
  function updateDateColors() {
    ...
    $.ajax({
      ...
      .done(function (response) {
        if (!response || response.success !== true) {
          showUpdateStatus(
            response?.message || "Automatic status update failed.",
            "error"
          );
          return;
        }
        ...
        if (updatedCount > 0) {
          showUpdateStatus(
            `Moved ${updatedCount} due tent request${updatedCount === 1 ? "" : "s"} to For Retrieval.`,
            "success"
          );
        ...
        if (skippedCount > 0) {
          showUpdateStatus(
            `${skippedCount} due request${skippedCount === 1 ? " was" : "s were"} skipped because assigned tent statuses need review.`,
            "warning"
          );
        }
      ...
      .fail(function (xhr, status) {
        const message =
          ...
          (status === "timeout"
            ? "Automatic status check timed out. It will retry shortly."
            : "Automatic status check failed. It will retry shortly.");
    ...
    void("=== DATE CHECKING SESSION STARTED ===");
    void("Today's date (Local):", today.format("YYYY-MM-DD HH:mm:ss"));
    void(
    ...
    try {
      $(".retrieval-date").each(function (index) {
        try {
          ...
          void("Raw tent text:", `"${tentText}"`);
          void("Parsed tent numbers:", tentNumbers);
          void("Current status:", `"${selectedValue}"`);
          void("Data ID:", id);
          ...
          void("Is today?", isToday);
          void("Is tomorrow?", isTomorrow);
          void("Is in the past?", isPast);
          ...
          void("FINAL DAYS DIFFERENCE USED:", finalDiffDays);
          void("Color status:", colorStatus);
          void("Action:", actionTaken);
      ...
      void("\n=== PROCESSING SUMMARY ===");
      void(`Total dates processed: ${processedDates.length}`);
      void(`Red date entries (overdue): ${redDates.length}`);
      void(`Orange date entries (due today): ${orangeDates.length}`);
      ...
      void("=== DATE CHECKING SESSION COMPLETED ===\n");
  ...
  function makeAjaxRequestWithRetry(ajaxConfig, requestType, retryCount = 0) {
    return new Promise((resolve, reject) => {
      ...
      $.ajax(ajaxConfig)
        .done(function (response) {
          void(`✅ AJAX success (${requestType}):`, response);
  ...
  function checkAllUpdatesComplete(completed, total) {
    try {
      void(`📊 Update groups completed: ${completed}/${total}`);

      if (completed >= total) {
        if (pendingUpdates.length > 0 && !queueProcessingInProgress) {
          void(`🔄 Processing ${pendingUpdates.length} queued updates`);
          ...
          updateInProgress = false;
          hideUpdateStatus();
          void("✅ All updates completed successfully");
        }
      }
    } catch (error) {
  ...
  function cleanupOldRetryAttempts() {
    try {
      ...
      if (false && cleanedCount > 0) {
        void(`🧹 Cleaned up ${cleanedCount} old retry attempts`);
      }
    } catch (error) {
...
// Popup/modal logic
$(function () {
  function normalizeBootstrapModalState() {
    window.setTimeout(function () {
      ...
      if (openModals.length === 1 && backdrops.length > 1) {
  ...
  document.addEventListener("hidden.bs.modal", normalizeBootstrapModalState);
  document.addEventListener("shown.bs.modal", normalizeBootstrapModalState);
  document.addEventListener("click", function (event) {
    if (event.target.closest('[data-bs-dismiss="modal"]')) {
      normalizeBootstrapModalState();
    }
  });
  window.addEventListener("pageshow", normalizeBootstrapModalState);
  ...
  $(document).on("click", ".viewButton", function () {
    ...
    $.get("../fetch_data.php", { id: id }, function (data) {
      ...
      $('#viewEditModal .box').removeClass('selected current-assignment').attr('aria-pressed', 'false');
      if (data.tent_no) {
        var tentNumbers = data.tent_no.split(',').map(n => parseInt(n.trim())).filter(n => !isNaN(n));
        tentNumbers.forEach(num => {
          $('#viewEditModal .box[data-box="' + num + '"]').addClass('selected current-assignment').attr('aria-pressed', 'true');
        });
        // Ensure the field reflects the selected boxes (sorted)
        $("#viewEditTentNo").val(tentNumbers.sort((a,b) => a - b).join(','));
      }
      updateEditTentSelectionUI();
      // Show the new Bootstrap 5 modal
      var modal = bootstrap.Modal.getOrCreateInstance(document.getElementById("viewEditModal"));
      modal.show();
    });
  });
  ...
  function updateBoxStatus() {
    $.get("../fetch_tent_status.php", function (data) {
      ...
      data.forEach(function (item) {
        ...
        var box = $('#viewEditModal .box[data-box="' + item.id + '"]');
        if (box.length) {
          box.removeClass("green red orange blue");
          if (item.Status === "On Stock" || item.Status === "Retrieved") {
            box.addClass("green").attr("aria-disabled", "false");
          } else if (item.Status === "Installed") {
            box.addClass("red").attr("aria-disabled", "true");
          } else if (item.Status === "For Retrieval") {
            box.addClass("orange").attr("aria-disabled", "true");
          } else if (item.Status === "Long Term") {
            box.addClass("blue").attr("aria-disabled", "true");
          }
        }
      });
      updateEditTentSelectionUI();
...
function updateEditTentSelectionUI(message) {
  var selectedCount = $("#viewEditModal .box.selected").length;
  var requestedCount = parseInt($("#viewEditNoOfTents").val()) || 0;
  $("#editSelectedTentCount").text(selectedCount);
  $("#editSelectedTentLabel").text(selectedCount === 1 ? "selected" : "selected");
  $("#viewEditModal .box").each(function () {
    $(this).attr("aria-pressed", $(this).hasClass("selected") ? "true" : "false");
  });
  $("#editTentSelectionMessage").text(
    message || (selectedCount > 0 && requestedCount && selectedCount !== requestedCount
      ? "Select " + requestedCount + " tent" + (requestedCount === 1 ? "" : "s") + " before saving."
      : "")
  );
}

$(document).on("click", "#viewEditModal .box", function () {
  if (!$(this).hasClass("green") && !$(this).hasClass("current-assignment")) return;
  var clickLimit = parseInt($("#viewEditNoOfTents").val()) || 1;
  var selectedBoxes = $("#viewEditModal .box.selected");
  var isSelected = $(this).hasClass("selected");
  if (!isSelected && selectedBoxes.length >= clickLimit) {
    updateEditTentSelectionUI("Selection limit reached. Increase the requested quantity to select more tents.");
    return;
  }
  ...
  var allSelectedNumbers = $("#viewEditModal .box.selected")
    .map(function () {
...
$("#viewEditNoOfTents").on("input", function () {
  ...
  if (selectedBoxes.length > clickLimit) {
    ...
    var selectedNumbers = $("#viewEditModal .box.selected")
      .map(function () {
        return $(this).data("box");
      })
      .get()
...
$("#viewEditModal").on("hidden.bs.modal", function () {
  $("#viewEditModal .box").removeClass("selected current-assignment is-filtered-out").attr("aria-pressed", "false");
  $("#viewEditTentNo").val("");
  $("#editTentSearch").val("");
  $("#viewEditForm").removeClass("was-validated");
  $("#editFormStatus, #editTentSelectionMessage").text("");
  updateEditTentSelectionUI();
});

$("#editTentSearch").on("input", function () {
  var query = $(this).val().trim();
  $("#viewEditModal .box").each(function () {
    $(this).toggleClass("is-filtered-out", query !== "" && !String($(this).data("box")).includes(query));
  });
});
...
$(function () {
  $("#viewEditForm").on("submit", function (e) {
    ...
    var hasPartialAssignment = selectedCount > 0 && requestedCount !== selectedCount;
    if (!form.checkValidity() || hasPartialAssignment) {
      form.classList.add("was-validated");
      updateEditTentSelectionUI(
        hasPartialAssignment
          ? "The selected tent count must match the requested quantity."
          : ""
      );
    ...
    var submitButton = $("#viewEditSubmit");
    submitButton.prop("disabled", true).html('<span class="spinner-border spinner-border-sm" aria-hidden="true"></span> Saving');
    $("#editFormStatus").text("Saving changes...");
    var formData = new FormData();
    ...
    $.ajax({
      ...
      success: function (response) {
        ...
        if (result.success === false) {
          $("#editFormStatus").text(result.message || "The record could not be updated.");
          return;
        }
        $("#editFormStatus").text("Changes saved.");
      ...
      error: function (xhr, status, error) {
        ...
        $("#editFormStatus").text(xhr.responseJSON?.message || "Failed to update tent data. Please try again.");
      },
      complete: function () {
        submitButton.prop("disabled", false).html('<i class="fas fa-save" aria-hidden="true"></i> Save Changes');
      },
    });
  });
});
