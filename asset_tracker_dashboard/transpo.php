...
require_once __DIR__ . '/db_asset.php'; // Assuming this file contains your database connection code
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_data'])) {
    ...
    try {
        ...
        $vehicle = input_string($_POST, 'name', 120);
        $driver = input_string($_POST, 'driver', 120);
        $date = input_date($_POST, 'datepicker');
        $purpose = input_string($_POST, 'purpose', 255);
        $requestor = input_string($_POST, 'comboInput', 255);
        $location = trim((string) ($_POST['other'] ?? '')) ?: input_string($_POST, 'Location', 255);
        mysqli_begin_transaction($conn);
        $stmt = db_execute($conn, 'INSERT IGNORE INTO requestingParty (requestor) VALUES (?)', 's', [$requestor]);
        mysqli_stmt_close($stmt);
        $stmt = db_execute($conn, "INSERT INTO Transportation(Plate_no, Date, Requestor, Vehicle, Driver, Purpose, Location, Status, Status1) VALUES (?, ?, ?, ?, ?, ?, ?, 'Stand By', 'Stand By')", 'sssssss', [$plateNo, $date, $requestor, $vehicle, $driver, $purpose, $location]);
        mysqli_stmt_close($stmt);
        mysqli_commit($conn);
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_vehicle'])) {
    ...
    require_once __DIR__ . '/validators.php';
    asset_validate_csrf();
    $plateNo = input_string($_POST, 'plate_no1', 30);
    $vehicle = input_string($_POST, 'type_vehicle', 120);
    $stmt = db_execute($conn, "INSERT INTO Vehicle(Plate_no, Name, Status) VALUES (?, ?, 'Stand By')", 'ss', [$plateNo, $vehicle]);
    mysqli_stmt_close($stmt);
    header('Location: transpo.php');
    exit;
}
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['save_driver'])) {
    ...
    require_once __DIR__ . '/validators.php';
    asset_validate_csrf();
    $name = input_string($_POST, 'driver1', 120);
    $stmt = db_execute($conn, 'INSERT INTO Drivers(Name) VALUES (?)', 's', [$name]);
    mysqli_stmt_close($stmt);
    header('Location: transpo.php');
    exit;
}
    ...
    <div class="content">
            ...
            document.addEventListener("DOMContentLoaded", function() {
                ...
                function checkScannedData(scannedData) {
                    ...
                    $.ajax({
                        ...
                        dataType: 'json',
                        success: function(response) {
                            if (response.success && (response.data?.result === 'exists' || response.data?.result === 'Arrived')) {
        ...
        <script>
            ...
            function updateRecord() {
                ...
                // Send the update to the final tent tracking handler.
                fetch('tent_tracking/update_data.php', {
                        method: 'POST',
                        body: formData
                    })
                ...
                if (confirm("Are you sure you want to delete this record?")) {
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            window.location.href = 'tent_tracking/tracking.php';
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        const showPopup = (hoverData, rect) => {
            ...
            hoverData.forEach(vehicle => {
                ...
                div.innerHTML = `<span class="vehicle-plate">${AssetApp.escapeHtml(vehicle.Plate_No)}</span> <span class="vehicle-name">${AssetApp.escapeHtml(vehicle.Name)}</span>`;
