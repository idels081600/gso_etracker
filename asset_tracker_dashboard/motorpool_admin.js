    ...
    .then((data) => {
      if (data.success) {
        ...
        data.data.forEach((vehicle) => {
          ...
          const safe = window.AssetApp?.escapeHtml || String;
          row.innerHTML = `
                <td>${safe(vehicle.plate_no)}</td>
                <td>${safe(vehicle.car_model || "-")}</td>
                <td>${safe(vehicle.office)}</td>
                <td>${safe(vehicle.status)}</td>
                <td>${safe(vehicle.old_mileage)}</td>
                <td>${safe(vehicle.latest_mileage)}</td>
                <td>${safe(vehicle.no_of_repairs)}</td>
                <td>${safe(vehicle.new_repair_date || "-")}</td>
                <td>${safe(vehicle.date_procured || "-")}</td>
                <td>
                  <div class="d-flex">
                    <button class="btn btn-sm btn-primary select-vehicle me-1" aria-label="Edit vehicle"
                            data-plate="${safe(vehicle.plate_no)}"
                            data-id="${safe(vehicle.id)}"
                            data-model="${safe(vehicle.car_model || "")}"
                            data-office="${safe(vehicle.office)}"
                            data-status="${safe(vehicle.status)}"
                            data-old-mileage="${safe(vehicle.old_mileage)}"
                            data-latest-mileage="${safe(vehicle.latest_mileage)}"
                            data-repairs="${safe(vehicle.no_of_repairs)}"
                            data-repair-date="${safe(vehicle.new_repair_date || "")}"
                            data-procured="${safe(vehicle.date_procured || "")}"
                            data-dispatch="${safe(vehicle.no_dispatch)}">
                      <i class="fas fa-edit"></i>
                    </button>
                    <button class="btn btn-sm btn-danger delete-vehicle" aria-label="Delete vehicle"
                      data-plate="${safe(vehicle.plate_no)}">
        ...
        cell.textContent = `Error: ${data.message || "Unable to load vehicles"}`;
  ...
  function showToast(message, isSuccess = true) {
    ...
    toastEl.innerHTML = `
          <div class="d-flex">
              ...
              ${window.AssetApp?.escapeHtml(message) || ""}
              </div>
    ...
    fetch("get_repair_data_motorpool.php", {
      ...
      .then((data) => {
        if (data.success) {
          ...
          populateEditModal(data.data?.repair || data.repair);
          // Show edit modal
