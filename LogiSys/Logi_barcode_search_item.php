...
try {
    ...
    $item_no = mysqli_real_escape_string($conn, trim($data['barcode']));
    ...
    // Using your actual table structure from the main file
    $query = "SELECT 
                item_no,
                item_name,
                current_balance,
    ...
    if ($item) {
        ...
        // Determine status based on balance (same logic as in your main file)
        if ($balance == 0) {
            $status = "Out of Stock";
            $statusClass = "danger";
        } else if ($balance <= 10) {
        ...
        $response = array(
            ...
            'item' => array(
                ...
                'barcode' => $item['item_no'], // Use item_no as barcode
        ...
        $response = array(
            ...
            'barcode' => $item_no,
            'suggestion' => 'Please check the item number and try again, or add this item to inventory.'
        );
    }
} catch (Exception $e) {
    $response = array(
