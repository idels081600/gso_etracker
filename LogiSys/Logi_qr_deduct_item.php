...
try {
    ...
    // Validate input
    if (!$data || !isset($data['item_no']) || !isset($data['quantity']) || !isset($data['requestor'])) {
        throw new Exception('Item number, quantity, and requestor are required');
    }

    $item_no = mysqli_real_escape_string($conn, trim($data['item_no']));
    $quantity = (int)$data['quantity'];
    $requestor = mysqli_real_escape_string($conn, trim($data['requestor']));
    ...
    if (empty($requestor)) {
        throw new Exception('Requestor name is required');
    }
