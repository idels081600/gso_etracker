...
require_once 'logi_display_data.php'; // or your database connection file
...
try {
    ...
    $reference_no = isset($data['reference_no']) ? mysqli_real_escape_string($conn, trim($data['reference_no'])) : '';
    ...
    if (empty($reference_no)) {
        throw new Exception('PO No./IB No. is required');
    }
    ...
    // Log the transaction with reference number
    $log_query = "INSERT INTO inventory_transactions 
                  (item_no, item_name, transaction_type, quantity, previous_balance, new_balance, PO_no_IB_no, created_at) 
                  VALUES (?, ?, 'ADDITION', ?, ?, ?, ?, NOW())";
    ...
    $response = array(
        'success' => true,
        'message' => "Successfully added {$quantity} {$item['unit']} to {$item['item_name']} (Ref: {$reference_no})",
        'transaction' => array(
            'item_no' => $item_no,
