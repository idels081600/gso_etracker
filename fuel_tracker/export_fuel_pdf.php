...
$query = "SELECT date, office, vehicle, fuel_type, liters_issued FROM fuel WHERE id IN ($ids) ORDER BY office ASC, date ASC";
...
// Group records by office and calculate total liters
$groupedData = [];
while ($row = mysqli_fetch_assoc($result)) {
    ...
    $groupedData[$office]['records'][] = $row;
    $groupedData[$office]['total_liters'] += (float)$row['liters_issued'];
}

// Iterate through grouped data to add to PDF
foreach ($groupedData as $officeName => $officeData) {
    // Office Sub-header
    ...
    // Table header for records within this office
