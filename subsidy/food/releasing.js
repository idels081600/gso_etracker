...
async function searchTricycle(tricycleNo) {
    try {
        const response = await fetch(`api_get_claims.php?beneficiary_code=${encodeURIComponent(tricycleNo)}`);
        ...
        if (data.success) {
            currentTricycleNo = data.data.beneficiary_code;
            currentDriverName = data.data.full_name || '';
            ...
            currentTricycleEl.textContent = data.data.beneficiary_code;
            currentStatusEl.textContent = `${data.data.status} | ${data.data.full_name}`;
            ...
            renderVoucherButtons(data.data.beneficiary_code, claimedVouchersList);
...
document.getElementById('confirmSubmit').addEventListener('click', async () => {
    ...
    try {
        const response = await fetch('api_claim_voucher.php', {
            ...
            body: JSON.stringify({
                benificiary_no: currentTricycleNo,
