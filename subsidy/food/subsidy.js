...
// Load today's fuel up count from voucher_claims table
        ...
        if (dailyData.success) {
            ...
            document.getElementById('todayBoats').textContent = dailyData.today_boats;
...
function updateDashboard(records) {
    ...
    const totalBoats = records.filter(r => r.tricycle_no.length > 4).length;
    ...
    const totalBoatVouchers = records.filter(r => r.tricycle_no.length > 4)
        .reduce((sum, r) => sum + parseInt(r.claimed_vouchers || 0), 0);

    // Calculate tricycle-specific metrics
    const tricycleRecords = records.filter(r => r.tricycle_no.length <= 4);
    const tricycleTotalVouchers = tricycleRecords.reduce((sum, r) => sum + parseInt(r.claimed_vouchers || 0), 0);
    const tricycleTotalLiters = tricycleRecords.reduce((sum, r) => sum + parseFloat(r.total_liters || 0), 0);

    // Calculate boat-specific metrics
    const boatRecords = records.filter(r => r.tricycle_no.length > 4);
    const boatTotalVouchers = boatRecords.reduce((sum, r) => sum + parseInt(r.claimed_vouchers || 0), 0);
    const boatTotalLiters = boatRecords.reduce((sum, r) => sum + parseFloat(r.total_liters || 0), 0);

    // Calculate grand totals
    const grandTotalVouchers = totalClaimed;
    const grandTotalLiters = tricycleTotalLiters + boatTotalLiters;
    ...
    document.getElementById('totalBoatVouchers').textContent = totalBoatVouchers;

    // Update summary statistics
    document.getElementById('tricycleTotalVouchers').textContent = tricycleTotalVouchers;
    document.getElementById('tricycleTotalLiters').textContent = tricycleTotalLiters.toFixed(2);
    document.getElementById('boatTotalVouchers').textContent = boatTotalVouchers;
    document.getElementById('boatTotalLiters').textContent = boatTotalLiters.toFixed(2);
    document.getElementById('grandTotalVouchers').textContent = grandTotalVouchers;
...
document.getElementById('updateLitersBtn').addEventListener('click', async () => {
    ...
    try {
        ...
        if (data.success) {
            ...
            document.getElementById('totalBoatLiters').textContent = data.total_boat_liters;
