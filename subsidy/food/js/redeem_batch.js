...
function setupEventListeners() {
    ...
    document.getElementById('clearVoucherSearch').addEventListener('click', () => {
        document.getElementById('voucherSearch').value = '';
        filterAvailableVouchers();
    });

    // Redeem button
    document.getElementById('redeemBtn').addEventListener('click', showConfirmModal);
...
function searchVendor() {
    ...
    fetch('api_search_vendor.php?vendor_serial=' + encodeURIComponent(vendorSerial))
        ...
        .then(data => {
            ...
            if (data.success) {
                const vendor = data.match_type === 'exact' ? data.vendor : data.vendors[0];
...
function loadVouchers(vendorId) {
    ...
    tbody.innerHTML = '<tr><td colspan="3" class="text-center py-3"><span class="spinner-border spinner-border-sm me-2"></span>Loading...</td></tr>';

    fetch('api_get_vendor_vouchers.php?vendor_serial=' + encodeURIComponent(currentVendor.vendor_serial))
        .then(res => res.json())
        .then(data => {
            if (data.success) {
...
function renderAvailableVouchers() {
    ...
    if (available.length === 0) {
        tbody.innerHTML = '<tr><td colspan="3" class="text-center text-muted py-3">No verified vouchers available</td></tr>';
    ...
    available.forEach(voucher => {
        ...
        const voucherCode = (voucher.beneficiary_code || 'N/A') + ' - 00' + voucher.voucher_number;
...
function renderSelectedVouchers() {
    ...
    if (selectedVouchers.length === 0) {
        tbody.innerHTML = '<tr><td colspan="4" class="text-center text-muted py-3">No vouchers selected</td></tr>';
    ...
    document.getElementById('selectedCount').textContent = selectedVouchers.length + ' selected';
}
...
function selectVoucher(voucherId) {
    ...
    if (voucher) {
        ...
        for (let i = 0; i < rows.length; i++) {
            const btn = rows[i].querySelector(`button[onclick="selectVoucher(${voucherId})"]`);
            if (btn) {
...
function showConfirmModal() {
    ...
    document.getElementById('modalVendorName').textContent = currentVendor.vendor_name + ' (' + currentVendor.vendor_serial + ')';
...
function confirmRedeem() {
    ...
    redeemBtn.innerHTML = '<i class="bi bi-hourglass-split me-1"></i> Processing...';
    ...
    .then(data => {
        ...
        if (data.success) {
            ...
            const formattedAmount = '₱' + data.batch.total_amount.toLocaleString('en-PH', {minimumFractionDigits: 2, maximumFractionDigits: 2});
