...
// Get current station if user has one assigned
...
if (mysqli_num_rows($check_result) > 0) {
    // User already has a station assigned - store it but allow change
    $station_data = mysqli_fetch_assoc($check_result);
    $current_station_id = $station_data['station_id'];
    $current_station_name = $station_data['station_name'];
    
    // Set session if not already set
    if (!isset($_SESSION['station_id'])) {
        $_SESSION['station_id'] = $current_station_id;
        $_SESSION['station_name'] = $current_station_name;
    }
}

// Fetch all active gas stations
$stations_sql = "SELECT * FROM gas_stations WHERE is_active = 1 ORDER BY station_name";
                    ...
                    <div class="card-body p-5">
                        ...
                        <form id="stationForm">
                            <div class="row g-3" id="stationList">
                                ...
                                ?>
                                <div class="col-12">
                                    <div class="card station-card <?php echo $is_current ? 'selected' : ''; ?>" data-station-id="<?php echo $station['id']; ?>" data-station-name="<?php echo htmlspecialchars($station['station_name']); ?>" data-current="<?php echo $is_current ? 'true' : 'false'; ?>">
                                        <div class="card-body d-flex align-items-center">
                                            <i class="bi bi-shop fs-3 text-warning me-3"></i>
                                            <div>
                                                <h5 class="mb-1"><?php echo htmlspecialchars($station['station_name']); ?></h5>
                                                <small class="text-muted"><?php echo htmlspecialchars($station['station_code']); ?></small>
                                                <?php if ($is_current): ?>
