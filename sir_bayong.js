...
$(document).ready(function () {
  ...
  $(document).on("click", ".delete-btn", function () {
    ...
    if (confirm("Are you sure you want to delete this record?")) {
      $.ajax({
        url: "delete_record_sir_bayong.php",
        type: "POST",
        data: { id: id },
        dataType: "json",
...
$(document).ready(function () {
  ...
  $('.btn-warning[name="save_data2"]').on("click", function (e) {
    ...
    $.ajax({
      url: "/gso_test/update_record_sir_bayong.php",
      type: "POST",
      data: {
        id: currentId,
        sr_no: $("#sr_no").val(),
...
$(document).ready(function () {
  $("#filter_button").click(function () {
    ...
    $.ajax({
      url: "filter_records_sir_bayong.php",
      type: "POST",
      data: {
...
$(document).ready(function () {
  $("#addtoprint").click(function () {
    ...
    $.ajax({
      url: "transfer_to_print_bayong.php",
      type: "POST",
      data: {
        ids: rowIds,
      },
...
$(document).ready(function () {
  $("#markAsPaid").click(function () {
    ...
    if (selectedIds.length > 0) {
      $.ajax({
        url: "update_po_payment_bayong.php",
        type: "POST",
        data: {
          ids: selectedIds,
          po_no: selectedPO,
...
$("#deleteAllPrint").click(function () {
  if (confirm("Are you sure you want to delete all print records?")) {
    $.ajax({
      url: "delete_all_print_bayong.php",
      type: "POST",
      success: function (response) {
        let result = JSON.parse(response);
        if (result.success) {
...
$(document).ready(function () {
  $("#confirmPdf").click(function () {
    window.open("generate_pdf_bayong.php", "_blank");
  });
});
