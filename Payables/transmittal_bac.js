...
document.addEventListener('DOMContentLoaded', function() {
            ...
            fetch('fetch_transmittal_row.php?id=' + encodeURIComponent(id))
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        document.getElementById('edit_project_name').value = data.row.project_name;
                        document.getElementById('edit_transmittal_type').value = data.row.transmittal_type;
                        document.getElementById('edit_office').value = data.row.office;
                        document.getElementById('edit_received_by').value = data.row.received_by;
                        document.getElementById('edit_winning_bidders').value = data.row.winning_bidders;
                        document.getElementById('edit_NOA_no').value = data.row.NOA_no;
                        document.getElementById('edit_COA_date').value = data.row.COA_date;
                        document.getElementById('edit_notice_proceed').value = data.row.notice_proceed;
                        document.getElementById('edit_deadline').value = data.row.calendar_days;
                        var editModal = new bootstrap.Modal(document.getElementById('editTransmittalModal'));
                        editModal.show();
                    } else {
    ...
    document.getElementById('editTransmittalForm').addEventListener('submit', function(e) {
        e.preventDefault();
        var form = e.target;
        ...
        .then(data => {
            if (data.success) {
                ...
                var editModal = bootstrap.Modal.getInstance(document.getElementById('editTransmittalModal'));
