...
if (!$existsStmt) {
    payables_log_error('CTO remarks source lookup prepare failed: ' . $conn->error);
...
if (!$stmt) {
    payables_log_error('CTO remarks update prepare failed: ' . $conn->error);
    payables_json_response(['success' => false, 'error' => 'Unable to update remarks right now.'], 500);
}

$stmt->bind_param('si', $remarks, $recordId);
if (!$stmt->execute()) {
    payables_log_error('CTO remarks update execute failed: ' . $stmt->error);
    $stmt->close();
    payables_json_response(['success' => false, 'error' => 'Unable to update remarks right now.'], 500);
}
$stmt->close();
